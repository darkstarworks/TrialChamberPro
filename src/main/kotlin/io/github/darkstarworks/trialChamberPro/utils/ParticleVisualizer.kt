 ...
 */
class ParticleVisualizer(private val plugin: TrialChamberPro) {

    private val activeVisualizations = mutableMapOf<UUID, ScheduledTask>()
    ...
    fun showBox(player: Player, min: Location, max: Location, particleSpacing: Double = 1.0) {
        ...
        val task = plugin.scheduler.runTaskTimer(Runnable {
            if (!player.isOnline) {
                stopVisualization(player)
                return@Runnable
            }
        ...
        }, 0L, 10L) // Update every 10 ticks (0.5s)
